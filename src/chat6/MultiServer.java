...
public class MultiServer
{
	...
	//생성자
	public MultiServer() {
		//클라이언트 이름과 출력스트림 저장할 HashMap생성
		clientMap = new HashMap<String, PrintWriter>();
		//HaspMap동기화 설정 쓰레드가 사용자정보에 동시에 접근하는것을 차단한다
	...
	public void init() {
		...
		try {
			...
			while(true) {
				...
				/*
				 클라이언트 메세지를 모든 클라이언트에게 전달하기 위한 쓰레드 생성
				 및 start
				 */
...
{
	//map에 저장된 객체 키값 먼저 얻어옴
	Iterator<String> it = clientMap.keySet().iterator();
	
	//저장된 객체(클라이언트) 갯수만큼 반복한다
	while(it.hasNext()) {
		try {
			// 각 클라이언트 PrintWriter객체를 얻어온다
...
class MultiServerT extends Thread {
		...
		public void run() {
			...
			try {
				...
				//접속자 제외한 나머지 클라이언트만 입장메세지를 받는다
				sendAllMsg("", name + "님이 입장하셨습니다");
				
				//현재 접속한 클라이언트를 HashMap에 저장한다
				clientMap.put(name, out);
				
				//HashMap에 저장된 객체 수로 접속자수 파악
				...
				/*
				 클라이언트가 접속을 종료하면 예외가 발생하게 되어 finally에서
				 넘어오게 된다 이때 "대화명"을 통해 remove()시켜준다
				 */
				
				clientMap.remove(name);
				sendAllMsg("", name + "님이 퇴장");
